(function() {
  var Category, Client, HttpError, Listing, Me, OAuth, Search, Shop, User, request, util,
  ...
  Client = (function() {
    ...
    Client.prototype.parseOptions = function(options) {
      ...
      if (options.token && options.secret) {
        ...
        for (key in options) {
          ...
          if (key === 'secret') {
            this.authenticatedSecret = value;
            _results.push(delete options[key]);
          } else {
    ...
    Client.prototype.getAuthenticated = function() {
      var callback, params, path, url, _i, _ref;
      path = arguments[0], params = 3 <= arguments.length ? __slice.call(arguments, 1, _i = arguments.length - 1) : (_i = 1, []), callback = arguments[_i++];
      url = this.buildUrl.apply(this, [path].concat(__slice.call(params)));
      console.log("==> Perform authenticated request on " + url);
      return (_ref = this.etsyOAuth).get.apply(_ref, [url, this.authenticatedToken, this.authenticatedSecret].concat(__slice.call(params), [(function(_this) {
