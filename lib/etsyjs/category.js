(function() {
  ...
  Category = (function() {
    ...
    Category.prototype.topLevelCategoryChildren = function(_arg, cb) {
      var secret, token;
      token = _arg.token, secret = _arg.secret;
      return this.client.get("/taxonomy/categories/" + this.tag, token, secret, function(err, status, body, headers) {
        if (err) {
          return cb(err);
        }
        if (status !== 200) {
